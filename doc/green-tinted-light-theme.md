...
| `home/gitconfig` | Base (dark) `[delta]` colours, plus the include for the light-mode overlay |
| `~/.gitconfig-presentation` | Light-mode delta overrides — generated by `bin/presentation-mode`, not tracked |
...
Run `presentation-mode` to toggle. It updates Ghostty (via config overlay +
SIGUSR2 reload), all running Neovim instances (via socket), tmux, and git-delta.

Delta uses the same optional-overlay pattern as Ghostty: light mode writes
`~/.gitconfig-presentation` with `light = true`, the `GitHub` syntax theme and
pale diff backgrounds; dark mode removes the file so the base `[delta]` block in
`home/gitconfig` applies. The dark values were retuned at the same time: the
previous diff backgrounds sat within 1–4 luminance points of the `#0a1f0a`
background, so whole added and removed lines were invisible. See
`doc/gitconfig.md` for the measured values, the include ordering, and the
`--includes` gotcha when checking values with `git config`.
